...
<table align='center' style='margin-top:40px; margin-bottom:40px;'>
	<tr>
		<td class='alldone' onclick="window.open('inc/invoicepdf.php?id=<?=encrypt($_SESSION['invoice']."-".md5($_SESSION['invoice']))?>')"><img src='style/images/save_pdf.png'/><br>Download PDF</td>
		<td class='alldone' onclick="window.open('inc/invoicehtml.php?id=<?=encrypt($_SESSION['invoice']."-".md5($_SESSION['invoice']))?>')"><img src='style/images/save_html.png'/><br>View HTML</td>
	</tr>
	<tr>
...
http://www.<?=$_SERVER['HTTP_HOST']?><?=str_replace('add.php','',$_SERVER['SCRIPT_NAME'])?>inc/invoicehtml.php?id=<?=encrypt($_SESSION['invoice']."-".md5($_SESSION['invoice']))?>			
Best Regards,
<?=$cfg['name']?>
		</textarea></td>
	</tr>
